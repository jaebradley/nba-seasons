...
def write_franchises(directory_path: str, history: FranchiseHistory) -> None:
    for franchise in history.history.keys():
        with open(
                directory_path + "/" + str(base64.b64encode(franchise.name.encode(encoding="utf-8")), "utf-8") + ".yaml",
                'w+',
                encoding="utf-8"
        ) as file:
            file.write("")
...
def write_teams(directory_path: str, history: FranchiseHistory) -> None:
    for franchise, teams in history.history.items():
        for team in teams:
            with open(
                    directory_path + "/" + str(base64.b64encode(team.name.encode(encoding="utf-8")), "utf-8") + ".yaml",
                    'w+',
                    encoding="utf-8"
            ) as file:
                # TODO: @jaebradley write this in YAML format
                file.write("franchise: " + franchise.name)
